		...
		try {
			...
			switch(command) {
				...
				case 'ep':
					...
					break
				case 'tahta':
					if (args.length < 1) return reply('omg o texto')
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falha ao converter ${tipe} para adesivo`)
							})
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('BOT', authorname)} ${ran} -o ${ran}`, async (error) => {
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie uma foto com uma legenda ${prefix}adesivo ou tag de imagem que foi enviado`)
					}
					break
				case 'gtts':
					if (args.length < 1) return client.sendMessage(from, 'Onde está o código do idioma??', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Onde está o texto omg?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					dtt.length > 600
					? reply('O texto é muito omg')
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ É transmitido ]\n\n${body.slice(4)}`})
						}
						reply('Sucesso na transmissão')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ É transmitido ]\n\n${body.slice(4)}`)
						}
						reply('Sucesso na transmissão')
					}
					break
					...
					if (mentioned.length > 1) {
						...
						mentions(`Sucesso Promote @${mentioned[0].split('@')[0]} Como administrador de grupo!`, mentioned, true)
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recebido, problema : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
                ...
                case 'leave':
                    ...
                    break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ responda sticker um ❌')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ Falha ao converter o sticker em imagem ❌')
